...
class Bot extends EventEmitter {
	...
	_handleMessage(payload) {
		if (this._handleConversationResponse('message', payload)) return;
		const text = payload.body;
		if (!text) return;
	...
	_handleConversationResponse(type, payload) {
		const threadID = payload.threadID;
		return this._conversations.some((convo) => {
			if (threadID && threadID === convo.threadID && convo.isActive()) {
				convo.respond(payload);
				return true;
			}
			return false;
		});
	...
	conversation(threadID, factory) {
		if (!threadID || !factory || typeof factory !== 'function') {
			throw new Error(`You need a threadID and a callback to start a conversation`);
		}
		const convo = new Conversation(this, threadID);
